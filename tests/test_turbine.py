...
def test_steam_tight_optimal():
    r = calc_regular_turbine("steam", EA, "Turbine", "Tight", "Steam", STEAM_FUELS["Steam"])
    assert r.opt_output_eu_t == 660
    assert r.opt_flow == 1200
    assert r.lifetime_s == pytest.approx(832980, abs=200)

def test_steam_loose_optimal():
    r = calc_regular_turbine("steam", EA, "Turbine", "Loose", "Steam", STEAM_FUELS["Steam"])
    assert r.opt_output_eu_t == 2123

# --- Regular Gas ---
def test_gas_tight_benzene():
    r = calc_regular_turbine("gas", EA, "Turbine", "Tight", "Benzene", GAS_FUELS["Benzene"]["eu_per_l"])
...
def test_plasma_tight_helium():
    ...
    r = calc_regular_turbine("plasma", EA, "Turbine", "Tight", "Helium Plasma", 81920)
    assert r.opt_output_eu_t == 58572
    assert r.opt_flow == 13  # L/s

# --- XL Steam ---
def test_xl_steam_loose_sc():
    r = calc_xl_turbine("steam", EA, "Turbine", "Loose", "SC Steam", STEAM_FUELS["SC Steam"], is_dense=True)
    assert r.opt_output_eu_t == pytest.approx(67932, abs=10)

# --- XL Gas ---
def test_xl_gas_tight_nitrobenzene():
    r = calc_xl_turbine("gas", EA, "Turbine", "Tight", "Nitrobenzene", GAS_FUELS["Nitrobenzene"]["eu_per_l"])
